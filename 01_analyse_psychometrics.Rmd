...
item <- read.csv(file.path(dir,"Factor_analysis_data_FO.csv"))


# only use complete data
complete_obs <- df$participant[df$Exclusion_H2 == 0]

# subset item-level data
...
```{r, echo = F}
# read in item-level data
item <- read.csv(file = file.path(dir, "Factor_analysis_data_FO.csv"))
item <- subset(item, participant %in% complete_obs)
...
# read in item-level data
item <- read.csv(file = file.path(dir, "FBenv_item-level.csv"))
item <- subset(item, participant %in% complete_obs)
