...
binned <- ggplot(tmp, aes(x = acc_change, y = fo)) + 
  geom_point(col = navy) +
  ambition_theme + 
...
ggsave(plot = binned,
       filename = file.path(cd, "FigureB1.jpg"), 
       units = "cm", width = 20, height = 12)

binned <- binned + ambition_theme_minimal

ggsave(plot = binned, 
       filename = file.path(cd, "FigureB1_skill.jpg"), 
       units = "px", width = 2160, height = 764, dpi = 300)


# create scatter plot with binned data
binned <- ggplot(tmp, aes(x = tot_change, y = fo)) + 
  geom_point(col = navy) +
  ambition_theme + 
...
ggsave(plot = binned,
       filename = file.path(cd, "FigureB2.jpg"), 
       units = "cm", width = 20, height = 12)

binned <- binned + ambition_theme_minimal

ggsave(plot = binned, 
       filename = file.path(cd, "FigureB2_tot.jpg"), 
       units = "px", width = 2160, height = 764, dpi = 300)
...
# create scatter plot with binned data
binned <- ggplot(tmp_l, aes(x = fo, y = value)) + 
  geom_point(col = navy) +
  ambition_theme + 
...
ggsave(plot = binned,
       filename = file.path(cd, "FigureB12.jpg"), 
...
ggsave(filename = file.path(cd, "Figure3_H1B.jpg"), 
...
ggsave(plot = plt, 
       filename = file.path(cd, "Figure3_H1B_skill.jpg"), 
       units = "px", width = 2160, height = 764, dpi = 300)
...
plt <- ggplot(data = beta[beta$DV == "Time off task", ], aes(x = `FO subscale`, y = beta)) +
  ...
  ambition_theme_minimal
ggsave(plot = plt, 
       filename = file.path(cd, "Figure3_H1B_tot.jpg"), 
       units = "px", width = 2160, height = 764, dpi = 300)
