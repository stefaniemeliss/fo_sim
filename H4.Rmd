...
wtd_2 <- c("FO2_wtd", 
           ...
           "FO2_aware_wtd", 
           "FO2_account_wtd")
construct <- c("FO", "FO self-efficacy", "FO utility", "FO social awareness", "FO accountability")

out <- c("FO", "FOselfeff", "FOutil", "FOaware", "FOaccount")
...
for (i in 1:length(construct)) {
  ...
  # select: summed score
  df_sum <- df %>%
    select(participant, sum_1[i], sum_2[i]) 
  # reshape wide -> long
  dfl_sum <- reshape2::melt(df_sum)
  # format 
  dfl_sum$variable <- ifelse(dfl_sum$variable == sum_1[i], "1", "2")
  dfl_sum$variable <- factor(dfl_sum$variable, levels = c("1", "2"))
  dfl_sum$construct <- construct[i]
  dfl_sum$comp <- "Summed score"
  
  # select: weighted score
  df_wtd <- df %>%
    select(participant, wtd_1[i], wtd_2[i]) 
  # reshape wide -> long
  dfl_wtd <- reshape2::melt(df_wtd)
  # format 
  dfl_wtd$variable <- ifelse(dfl_wtd$variable == wtd_1[i], "1", "2")
  dfl_wtd$variable <- factor(dfl_wtd$variable, levels = c("1", "2"))
  dfl_wtd$construct <- construct[i]
  ...
  if (construct[i] == "FO" | construct[i] == "FO self-efficacy") {
    ...
    # add axis labels
    if (construct[i] == "FO self-efficacy") {
      main <- main + xlab("Survey") + ylab(construct[i])
    } else {
      main <- main + xlab("") + ylab(construct[i])
    }
    
    # Store object in the global environment.
    assign(paste0("plt_", out[i]), main)
  ...
  # add axis labels
  if (construct[i] == "FO self-efficacy") {
    appendix <- appendix + xlab("Survey") + ylab(construct[i])
  } else {
    appendix <- appendix + xlab("") + ylab(construct[i])
  }
  
  # Store object in the global environment.
  assign(paste0("spagh_", out[i]), appendix)
